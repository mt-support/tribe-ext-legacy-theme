<?php
/**
 * Plugin Name:       The Events Calendar Extension: legacy-theme
 * Plugin URI:        
 * GitHub Plugin URI: https://github.com/mt-support/tec-labs-legacy-theme
 * Description:       
 * Version:           1.0.1
 ...
 */
define( 'TRIBE_EXTENSION_LEGACY_THEME_FILE', __FILE__ );
 ...
 * @since 1.0.0
 */
function tribe_extension_legacy_theme() {
	...
	Tribe__Autoloader::instance()->register_prefix(
		'\\Tribe\\Extensions\\Legacytheme\\',
		__DIR__ . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'Tec',
		'legacy-theme'
	);

	// Deactivates the plugin in case of the main class didn't autoload.
	if ( ! class_exists( '\Tribe\Extensions\Legacytheme\Plugin' ) ) {
		tribe_transient_notice(
			'legacy-theme',
			'<p>' . esc_html__( 'Couldn\'t properly load "The Events Calendar Extension: legacy-theme" the extension was deactivated.', '__TRIBE_DOMAIN__' ) . '</p>',
			[],
	...
	tribe_register_provider( '\Tribe\Extensions\Legacytheme\Plugin' );
}

// Loads after common is already properly loaded.
add_action( 'tribe_common_loaded', 'tribe_extension_legacy_theme' );
